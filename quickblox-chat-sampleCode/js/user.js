...
function User() {
    ...
    var _getUsers = function (args) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var colback = function (err, responce) {
                ...
                var arrList=[{firstName: 'Dikshita', lastName: 'Pant', sapid: '7722112541'}]
                var userFilteredList = responce.items.filter(function(data){
                    for(var i=0;i<=arrList.length-1;i++)
                    {
                        var sapid=arrList[i].sapid;
                        if(sapid==data.user.login)
                        return true;
                    }
                })
                var userList = userFilteredList.map(function(data){
                    return self.addToCache(data.user);
                });
                
                //[{user: {id: 135018776, full_name: 'Dikshita Pant', login: '77221125416'}}]
     ...
     * @param {boolean} [renderUsers=true] whether to render fetched users (`true` by default)
     */
    ...
    this.scrollHandler = function (e) {
        ...
        console.log(shouldLoadNextPage)
...
User.prototype.initGettingUsers = function (userListConteiner, userListFilter, params) {
    ...
    console.log("ganesh")
    console.log(this.userListConteiner)
    console.log(this.userListFilter)
    console.log(params)
